 ...
 *
 * <p>{@code key} is the short human prefix that item keys are minted from ({@code VEC}
 * → {@code VEC-41}). It is held separately from the identifier precisely so it can be
 * changed without rewriting references: item keys store the prefix at mint time, so
 * re-keying a workspace is a display concern, not a data migration (VEC-11).
 */
